 ...
 * <p>创建日期：2016年12月27日 下午2:19:30</p>
 * <p>类全名：qrcode.QRCode</p>
 * 作者：赵玉柱
 * 初审：
 * 复审：
 * 监听使用界面:
...
public class QRCode
{
	...
	{
		int imageTypeIndex = imagePath.indexOf('.');
		if (imageTypeIndex <= 0)
		{
			throw new RuntimeException("文件没有类型");
		}
	...
	private BufferedImage qRCodeCommon(String content, String imageType, int size)
	{
		...
		try
		{
			...
			gs.setFont(new Font("abd", Font.BOLD, 12));
			gs.drawString("dsadsadsa", 0, 0);
			gs.dispose();
			bufImage.flush();
		} catch (Exception e)
		{
